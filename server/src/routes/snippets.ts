import { Role, SharePermission, type Prisma } from "@prisma/client";
...
function snippetInclude(userId: string) {
  return {
    owner: {
      select: {
        id: true,
...
function visibilityFilter(userId: string, role: Role): Prisma.SnippetWhereInput {
  if (role === Role.ADMIN) {
...
async function getSnippetOrThrow(id: string) {
...
async function assertReadable(snippetId: string, userId: string, role: Role): Promise<void> {
  if (role === Role.ADMIN) {
    return;
  }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const filters: Prisma.SnippetWhereInput[] = [visibilityFilter(req.user!.id, req.user!.role)];
